...
policy_document = """
    I have a policy document 
    policy document = Marketing Treasury-based services
...
def analyse_with_llm(body):
    url_for_compliance = body.get("url")
    html = urlopen(url_for_compliance)
    soup = BeautifulSoup(html, 'lxml')
    url_data = soup.get_text()
    url_data_clean = url_data.strip()
    index = index_builder(url_for_compliance,url_data_clean)
    response = query_builder(index)
...
def index_builder(url, url_data):

    docs = []
    model = "gpt-4"
    role = "compliance"
    docs.append(doc_builder(url, url_data))
...
def query_builder(index):


    final_query = f"Use the information in the policy document {policy_document} \n" \
                   f" Detect the non compliant findings in the index. \n At the end mention solution of each of the issues."}
